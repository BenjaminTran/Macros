...
{
    return xi_bgfunc( x,par ) + sgfunc( x,&par[2] );
}

Double_t V0_total( Double_t *x, Double_t *par )
{
    return V0bgfunc( x,par ) + sgfunc( x,&par[4] );
}
...
void InvMassFit(  )
{
    ...
    */
    ...
    for( unsigned j=0; j<FILES.size(  ); j++ )
    {
        ...
        //TH2D* MassPt = ( TH2D* )FILES[j]->Get( "xiCorrelation/MassPt" );
        ...
        TF1* sigfit_ = new TF1( Form( "sigfit_%d",j ), sgfunc,1.26,1.4,5 );
        //sigfit_->SetNpx( 50 );
        sigfit_->SetNpx( 250 );
        ...
        for( int i=0; i<PtBinSize; i++ ){
            ...
            SigFit.push_back( new TF1( Form( "sigfit_%d",j*PtBinSize + i ), sgfunc, 1.26,1.4,7 ) );
