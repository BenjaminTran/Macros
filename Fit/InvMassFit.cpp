...
void InvMassFit(  )
{
    ...
    //
    //Define divided hist
    TH1D *dPhiPeak = new TH1D( "dPhiPeak", "#Xi - h^{#pm} ", 31, -( 0.5 - 1.0/32 )*PI, ( 1.5 - 1.0/32 )*PI  );
    ...
    //Project Phi
    TH1D* hbPhiTotPeak = hbackgroundPeak->ProjectionY( "PhiBkgTotPeak", 0, 10 );
    TH1D* hbPhiPosPeak = hbackgroundPeak->ProjectionY( "PhiBkgPosPeak", 23, -1 );
    TH1D* hsPhiTotPeak = hsignalPeak->ProjectionY( "PhiSigTotPeak", 0, 10 );
    TH1D* hsPhiPosPeak = hsignalPeak->ProjectionY( "PhiSigPosPeak", 23, -1 );

    //TH1D* hbPhiTotSide = hbackgroundSide->ProjectionY( "PhiBkgTot", 0, 10 );
    //TH1D* hbPhiPosSide = hbackgroundSide->ProjectionY( "PhiBkgPos", 23, -1 );
    //TH1D* hsPhiTotSide = hsignalSide->ProjectionY( "PhiSigTot", 0, 10 );
    //TH1D* hsPhiPosSide = hsignalSide->ProjectionY( "PhiSigPos", 23, -1 );

    TH1D* hbHadPhiTot = hBackgroundHad->ProjectionY( "PhiBkgHadTot", 0, 10 );
    TH1D* hsHadPhiTot = hSignalHad->ProjectionY( "PhiSigHadTot", 0, 10 );
    TH1D* hbHadPhiPos = hBackgroundHad->ProjectionY( "PhiBkgHadPos", 23, -1 );
    TH1D* hsHadPhiPos = hSignalHad->ProjectionY( "PhiSigHadPos", 23, -1 );

    TH1::SetDefaultSumw2(  );

    
    hbPhiTotPeak->Add( hbPhiPosPeak );
    hsPhiTotPeak->Add( hsPhiPosPeak );

    //hbPhiTotSide->Add( hbPhiPosSide );
    //hsPhiTotSide->Add( hsPhiPosSide );

    hbHadPhiTot->Add( hbHadPhiPos );
    hsHadPhiTot->Add( hsHadPhiPos );
    ...
    cout << "---------------------------------" << endl;
    cout << "The V2 for xi-h is " << FourierFitXi->GetParameter( 2 ) << endl;
    cout << "---------------------------------" << endl;
